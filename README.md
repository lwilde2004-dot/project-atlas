...
**The phone-facing agent is the least trusted client, so it gets the smallest surface that still does the job.** The version-controlled tool policy allows the four Atlas tool ids and denies web search, web fetch, exec, process and browser outright. Three of those four have side effects, so the boundary is not the tool list on its own: the capability launcher runs only names in a fixed registry, always with an argument list and never through a shell, and the listener only accepts messages from one Telegram id. The worst a hostile message achieves is a research run nobody asked for, not code execution.
